...
Version 1.0 | Status: Draft Platform: Flutter (single codebase, role-based access) · Firebase · Stripe
...
Location	geolocator (device GPS) + 2GIS Maps (dgis_mobile_sdk_map) for map display + Firestore live updates (Phase 1); native foreground service (Phase 2)	2GIS chosen for strong CIS/Central Asia coverage; requires a dgissdk.key file from dev.2gis.ru, not a plain API string. Map access is wrapped behind a domain MapProvider/LocationRepository interface (see Volume 3) so the provider stays swappable.
