...
Version 1.0 | Status: Draft Scope: Functional & Non-Functional Requirements — Phase 1 (MVP)
...
UC-AUTH-06	User can request a password reset email from the login screen; on submission, a reset link is sent via Firebase Auth (sendPasswordResetEmail) if the account exists — the UI shows the same generic confirmation regardless (no email-enumeration signal, consistent with UC-AUTH-04).
UC-AUTH-07	After registration, a verification email is sent automatically (sendEmailVerification). The user's shell displays a persistent, dismissible-per-session banner ("Please verify your email") while emailVerified == false, but is NOT blocked from using the app in Phase 1 (soft requirement, not a hard gate).
...
Register: invalid email format	Client-side regex check before submit (^[^@\s]+@[^@\s]+\.[^@\s]+$); server-side Firebase invalid-email is also mapped to the same message as a fallback.
Register: weak password	Minimum 8 characters, at least 1 digit, checked client-side before submit; Firebase weak-password mapped as fallback.
Register: email already in use	Firebase email-already-in-use mapped to "An account with this email already exists."
Register/Login: too many attempts	Firebase too-many-requests mapped to "Too many attempts. Please try again later."
Phone number entry	Collected with a country-code picker; stored in E.164 format (+<countrycode><number>) in Firestore, never as a raw local-format string.
